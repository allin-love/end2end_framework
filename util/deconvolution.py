...
def tikhonov_inverse(y, g, beta, eps=1e-2):
    ...
    return irfft_2(X, 2, signal_sizes=g.shape[1:3])
...
# def apply_tikhonov_inverse(captimg, psf, reg_tikhonov, apply_edgetaper=False):
#     """
#
#     Args:
...
#     Fpsf = rfft_2(fftshift(psf, dims=(-1,-2)), 2)  #[1,9,384,193,2]
#     Fcaptimgs = rfft_2(captimg, 2) #[b,9,384,193,2]
#     Fpsf = Fpsf.unsqueeze(2).unsqueeze(0)  # add shot dim  #shot dim will be swallowed, let batchsz = batchsz * num_shots
#     Fcaptimgs = Fcaptimgs.unsqueeze(1)  # add channel dim
#     b,c,s,h,w = Fcaptimgs.shape[:5]
#     est_X = tikhonov_inverse_fast(Fcaptimgs.view(b*s,c,1,h,w,-1), Fpsf, v=None, beta=0, gamma=reg_tikhonov,
#                                   dataformats='BCSDHW')
#     est_volumes = irfft_2(est_X, 2, signal_sizes=captimg.shape[-2:]).view(b,c*s,h,-1)
#     return est_volumes
...
def apply_tikhonov_inverse(captimg, psf, reg_tikhonov, apply_edgetaper=False):
    ...
    if apply_edgetaper:
        # Edge tapering
        captimg = edgetaper3d(captimg, psf)
    Fpsf = rfft_2(fftshift(psf, dims=(-1,-2)), 2)
    Fcaptimgs = rfft_2(captimg, 2)
    Fpsf = Fpsf.unsqueeze(2).unsqueeze(3)  # add shot dim
    Fcaptimgs = Fcaptimgs.unsqueeze(2)  # add shot dim
    est_X = tikhonov_inverse_fast(Fcaptimgs, Fpsf, v=None, beta=0, gamma=reg_tikhonov,
                                  dataformats='BCSDHW')
    est_volumes = irfft_2(est_X, 2, signal_sizes=captimg.shape[-2:])
    return est_volumes
